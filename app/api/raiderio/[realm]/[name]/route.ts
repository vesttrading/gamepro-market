...
export async function GET(
  ...
  try {
    ...
    const rioScore = Math.round(data.mythic_plus_scores_by_season?.[0]?.scores?.all  data.mythic_plus_scores_by_season?.scores?.all  0);
    ...
    const hasCE = progress ? progress.mythic_killed === progress.total_bosses && progress.total_bosses > 0 : false;
